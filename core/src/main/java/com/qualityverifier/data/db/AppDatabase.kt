...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /**
         * Adds the four `sessions` location columns.
         *
         * Null on every existing row, which is the honest answer: those assessments were
         * made before anybody was asked whether to record where they happened.
         */
        val MIGRATION_8_9 = object : Migration(8, 9) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE sessions ADD COLUMN latitude REAL")
                db.execSQL("ALTER TABLE sessions ADD COLUMN longitude REAL")
                db.execSQL("ALTER TABLE sessions ADD COLUMN locationAccuracyM REAL")
                db.execSQL("ALTER TABLE sessions ADD COLUMN locationAt INTEGER")
            }
        }
